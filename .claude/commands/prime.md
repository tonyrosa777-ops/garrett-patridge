...
#   Garrett Partridge     → e.g. "The Enchanted Collective"
#   garrettpartridge.com            → e.g. "enchantedmadison.com"
#   fractional COO and operations architect for defense-industrial-base manufacturers     → e.g. "luxury glamping and romantic experience property"
#   Brookline, NH (serving Greater Boston & Southern NH)          → e.g. "Madison, Indiana"
#   June 9, 2026 demo (production launch TBD)     → e.g. "June 2026"
#   owner/founder/GM of small-to-mid-size New England defense industrial base manufacturers  → e.g. "romantic couples aged 27–45 within 90 min drive"
#   fractional COO engagements, 30-day operational triage, and strategic consultancy installing Standard Work 2.0™        → e.g. "glamping tents, cottage, day-use hot tub escapes, proposal packages"
#   establish Garrett as New England's defense-manufacturing operations authority and convert trust into waitlisted strategic conversations          → e.g. "drive direct bookings and own the romantic getaway category in southern Indiana"
#   Calendly    → e.g. "Lodgify (launch) / OwnerRez (scale)"
#   ProfessionalService       → e.g. "LodgingBusiness" or "LocalBusiness" or "ProfessionalService"

---

You are the orchestrator for the **Garrett Partridge** (garrettpartridge.com) website build.
...
3. Read C:\Projects\Second Brain\Wealth\Optimus\knowledge\build-log.md (patterns + retrospectives tables only)
```
...
| Variable | Source |
|----------|--------|
| Garrett Partridge | initial-business-data.md Section 1 |
| garrettpartridge.com | initial-business-data.md Section 1 |
| fractional COO and operations architect for defense-industrial-base manufacturers | initial-business-data.md Section 1 — one-phrase description |
| Brookline, NH (serving Greater Boston & Southern NH) | initial-business-data.md Section 1 |
| June 9, 2026 demo (production launch TBD) | initial-business-data.md Section 7 |
| owner/founder/GM of small-to-mid-size New England defense industrial base manufacturers | initial-business-data.md Section 3 — specific, not generic |
| fractional COO engagements, 30-day operational triage, and strategic consultancy installing Standard Work 2.0™ | initial-business-data.md Section 2 |
| establish Garrett as New England's defense-manufacturing operations authority and convert trust into waitlisted strategic conversations | initial-business-data.md Section 5 |
| Calendly | initial-business-data.md Section 5 — or "TBD — confirm before Phase 4" |
| ProfessionalService | infer from business type — LocalBusiness / ProfessionalService / LodgingBusiness |
...
**Business Type:** fractional COO and operations architect for defense-industrial-base manufacturers
**Launch Target:** June 9, 2026 demo (production launch TBD)
**Last Updated:** [DATE]
...
> **Checkpoint schema (anchored — context-engineering).** Each session entry uses these fixed
> slots so a checkpoint survives a context reset without re-derivation. Summarize only what is
> NEW this session and APPEND a new entry — never rewrite prior entries (anchored, not
> wholesale-regenerated; prevents drift). See CLAUDE.md → Claude Code Operating Hygiene Rule and
> `Optimus Academy/concepts/context-engineering.md`.

### Session 1 — [Date]
**Intent:** <the one goal of this session, in a sentence>
**Completed:** <what concretely shipped>
**Files touched:** <full paths + the function/component/section changed in each — e.g. `src/app/services/page.tsx` (ServiceGrid), `src/data/site.ts` (services[]). This slot is what makes the checkpoint resumable.>
**Decisions Made:** <each non-obvious choice + the WHY behind it>
**Discovered:** <surprises, new constraints, things to remember>
**Current State:** <what works now, what is half-done>
**Next Session Starts At:** <the exact next step>
**Blockers:** <external blockers only, or "none">
```
...
Read agent file: C:\Projects\Second Brain\.claude\agents\onboarding\market-researcher.md
...
Read agent file: C:\Projects\Second Brain\.claude\agents\onboarding\design-synthesizer.md
Spawn with Agent tool (subagent_type: "general-purpose")
Pass in prompt: PROJECT_FOLDER = [PROJECT_FOLDER]
Wait for completion (run_in_background: false)
Verify output:
...
1. Read `C:\Projects\Second Brain\Wealth\Optimus\00 — Empire Index\absolute-rules-index.md` in full. This is the canonical enumeration of every CLAUDE.md absolute rule with its operational test.
...
6. **If all tests PASS:** the plan is rule-*legal* — now run the Premortem Plan Gate below to confirm it's actually *good* before scaffolding. Log "Absolute-Rule Cross-Check PASS" in progress.md with the timestamp.
...
**Premortem Plan Gate (runs AFTER the cross-check passes, BEFORE approving + scaffolding — CLAUDE.md Plan Premortem Gate Rule, Pattern #101):**

The cross-check catches *rule* violations; the premortem catches *strategic* failure a rule-legal plan can still have (won't convert, wrong audience, generic, mispositioned). Do NOT approve by asking "is this plan good?" — that elicits sycophantic validation.

1. **Spawn a fresh critic agent** (subagent_type: "general-purpose") — a clean reviewer is less anchored than the orchestrator that just shepherded the plan. Pass it ONLY the project folder + prompt B from `knowledge/patterns/premortem-plan-gate.md` (the build-plan premortem). It reads design-system.md + initial-business-data.md + market-intelligence.md and returns the ranked failure-mode autopsy (Conversion / Audience-fit / Credibility & taste / Positioning / Technical), each with the now-visible warning sign + the one plan change to prevent it, ranked by likelihood × cost-to-fix-later.
2. Append the ranked failure list to progress.md.
3. **Patch the plan NOW** for every high-likelihood × high-cost failure mode (Fix-on-Sight Rule — spawn a correction note back to design-synthesizer / adjust the section rhythm / update the copy brief). Logging a fix as "watch for this during build" is theater and is not acceptable.
4. If the autopsy reads affirming/comfortable, the critic slipped into validation — re-run with a harder framing ("it failed badly and publicly"). 
5. Re-approve the *patched* plan. Log "Premortem Plan Gate PASS — N failure modes surfaced, M patched" with the timestamp, then proceed to Stage 1C.
...
4. Create full directory structure — stub all files. For any personal-brand / solo-operator
   client (coach, consultant, agent, attorney, therapist, speaker, creator — anyone where the
   person IS the business), include `src/components/StructuredData.tsx` (site-wide Person +
   Organization JSON-LD with `sameAs`) as a DEFAULT scaffold stub, rendered once in
   `app/layout.tsx <body>` — alongside `metadataBase` + OpenGraph in the layout `metadata`
   export. It is populated by seo-aeo-specialist in Stage 1F from the canonical positioning
   string + `sameAs` URLs captured at intake (Pattern #100). Scaffolding the entity graph
   upfront avoids the Andrea Abella Marie post-launch retrofit, where the site shipped with
   none of it and the owner was unfindable for her own name.
...
```bash
# Site
NEXT_PUBLIC_SITE_URL=https://garrettpartridge.com
...
```
Read agent file: C:\Projects\Second Brain\.claude\agents\build\content-writer.md
...
```
Read agent file: C:\Projects\Second Brain\.claude\agents\build\animation-specialist.md
Spawn with Agent tool (subagent_type: "general-purpose", run_in_background: true)
Pass in prompt: PROJECT_FOLDER = [PROJECT_FOLDER]
...
19. Route structure — /service-areas/[city]:
    ...
    A. src/app/service-areas/page.tsx — index page
       ...
       - generateMetadata: title "Service Areas | Garrett Partridge"
    ...
    C. src/app/service-areas/[city]/page.tsx — individual city page (SERVER component)
       ...
       - generateMetadata({ params }): unique title + description per city
         title: "[Service] in [City], [State] | Garrett Partridge"
...
20. Homepage quiz CTA component — inline multi-step, launches without leaving the page
    Flow: hook headline + start CTA → step-by-step questions → instant on-screen result.
    No email gate, no lead-capture step — the result shows immediately, CTA = book the 1:1.
    ...
    Result screen: archetype name + tagline + body + recommended service card, then the
    inline <BookingCalendar /> directly below — the CTA is to book the one-on-one, right there.
    No name/email/phone fields anywhere in the quiz. No /api/quiz route. Calendly collects
    name + email during the booking step, so nothing is lost.
...
Commit: feat(quiz): multi-step quiz with emoji options — homepage CTA + /quiz page, inline booking on results
...
**Shop — ⛔ SKIPPED FOR THIS PROJECT (Anthony directive 2026-06-07). DO NOT EXECUTE THE BLOCK BELOW.**
No shop scaffold, no cart/CartDrawer/printful/stripe files, no `/shop` route, no nav link, no
sitemap entry, no homepage shop teaser. Freed capacity is reinvested into the expanded blog
slate (full market-intel §6 9-article AEO slate + extra SEO/AEO/GEO articles). Overrides the
Always-Built shop rule for this build only. Authority: progress.md → Build-Scope Decisions +
CLAUDE.md header. The original instructions below are retained for reference only — skip them.

**Shop (always scaffold first, then decision gate) — REFERENCE ONLY, SKIPPED:**
...
Read agent file: C:\Projects\Second Brain\.claude\agents\build\seo-aeo-specialist.md
...
Output files: sitemap.ts, robots.ts, opengraph-image.tsx files, JSON-LD components
              (incl. site-wide StructuredData.tsx — Person + Organization sameAs graph
              for personal-brand / solo-operator clients, Pattern #100)
```
...
- Personal-brand / solo-operator clients: site-wide Person + Organization JSON-LD with `sameAs`
  is present in the RENDERED HTML (curl, not just the source file); `sameAs` URLs byte-match the
  social profiles in initial-business-data.md Section 5; `Person.worksFor` ↔ `Organization.founder`
  cross-referenced by @id → BLOCK if missing (Pattern #100). No global `title.template` stacked on
  pages that already embed the brand (no double-branding, Pattern #93/#100).
...
### STAGE 1G.5 — Prospect-Journey Conversion Audit (funnel gate, runs before the pre-launch audits)

The build is feature-complete (1A-1G). Before the file-level and browser gates, audit the
CONVERSION FUNNEL across the business's real audiences. This is where structural IA/conversion
gaps get surfaced and fixed, so the later gates verify the corrected funnel, not the first draft.

Spawn a read-only agent (Explore or general-purpose) using the trigger-prompt template in:
  knowledge/patterns/prospect-journey-conversion-audit.md

Inputs the agent MUST use:
- Personas DERIVED from market-intelligence.md + initial-business-data.md + any client transcripts —
  every primary segment AND at least one under-served/edge audience the site might ignore (the
  biggest revenue leak usually hides there).
- Simulate against the ACTUAL rendered copy (read site.ts/components, or walk the live site with Playwright).

The agent returns, per persona: entry mindset → exact route → candid thoughts + friction →
"this is for me / not for me" → convert-or-bounce verdict (+ % confidence) → single biggest fix;
plus a ranked conversion/IA fix list with the highest-leverage change named.

The orchestrator then:
1. Reviews the convert/bounce verdicts.
2. Applies the in-scope fixes (new audience pages, re-routing, de-repelling copy, a persona
   selector, quiz/CTA corrections) — substantial work delegated per the Subagent Delegation Rule.
3. Re-runs the relevant slice (or a content re-walk) to confirm the leaks are closed.

CLAUDE.md authority: Prospect-Journey Conversion Audit Rule. Stance: the AUDITING hat of the
Operating Mindset Rule (Two Hats). Re-run whenever positioning or IA changes.

Exit: every research-derived persona has a clear "this is for me" path to the primary conversion,
and no audience the business WANTS bounces on repelling copy or a dead-end route.

Update progress.md: Stage 1G.5 complete — prospect-journey audit run, [N] conversion fixes applied.
Proceed to Stage 1H.
...
Read agent file: C:\Projects\Second Brain\.claude\agents\launch\pre-launch-auditor.md
Spawn with Agent tool (subagent_type: "general-purpose", run_in_background: false)
Pass in prompt: PROJECT_FOLDER = [PROJECT_FOLDER]
...
**Full playbook:** `C:\Projects\Second Brain\Wealth\Optimus\knowledge\patterns\end-of-build-multi-breakpoint-browser-audit.md`
...
Capture is the vault-central disk script — **`mcp__playwright__*` is retired from the
audit path** (Error #82). Subagents do not drive the browser; the orchestrator runs
the script directly. Full mechanics, gotchas, and exit-criteria detail live in the
pattern doc above — **do not re-embed them here** (single-source; Error #77).

**Setup**
1. `cd [PROJECT_FOLDER]/<next-app-folder>` → `npm run dev` with `run_in_background: true`
2. Poll `localhost:3000` (or read the task output for `✓ Ready`) before capturing
3. **Save the background task ID** — required for `TaskStop` at the end

**Capture — one command, writes every artifact to /audits/**
4. ```
   node "C:\Projects\Second Brain\Wealth\Optimus\tools\audit-capture.mjs" \
     --out "[PROJECT_FOLDER]/<next-app-folder>/audits" \
     --url http://localhost:3000 \
     --wait "<hero H1 tagline>"
   ```
   `--wait` is the hero H1 (`siteConfig.tagline` in `site.ts`) or a unique substring of
   it — it only renders post-hydration; static shell text yields blank screenshots. The
   script captures all four viewports + scrolled + nav-drawer + reduced-motion with
   per-viewport failure isolation and writes `manifest.json`.

**Analysis — disk facts first, vision only when flagged (per the pattern doc)**
5. Read as TEXT (no vision): `manifest.json` (all four succeeded?), `console-*.txt` (0/0),
   `tones-*.txt` (strict alternation, ends opposite footer), `overflow-*.txt` (`false`).
6. Load PNGs selectively: `-fold.png` for Half-Eye + hero-above-fold + nav overlay;
   `-full.png` for orphans/rhythm + blog/gallery image subject+era; `-rm.png` for
   reduced-motion gradient check.

**Exit criteria (all must be true — orchestrator verifies)**
- [ ] `manifest.json`: all four viewports succeeded
- [ ] 0 console errors AND 0 warnings at every viewport (`console-*.txt`)
- [ ] `tones-*.txt` strictly alternates, no DD/LL, ends opposite the footer (Pattern #98 / Error #79)
- [ ] `overflow-*.txt` is `overflow=false` at every viewport (esp. 375)
- [ ] Hero fits above the fold at every mobile width (`-fold.png`)
- [ ] No H1 orphan lines at any mobile width (`-full.png`)
- [ ] Mobile nav drawer overlay dark and opaque (`mobile-390-nav-open.png`)
- [ ] Reduced-motion: every section still a gradient (`-rm.png`)
- [ ] Every interior page has a brand-appropriate ambient animation (Animation Depth)
- [ ] Half-Eye blur-squint passes on every hero `-fold.png`

**Fix loop (if any issue found)**
7. Fix → **re-run `audit-capture.mjs`** (regenerates all artifacts) → re-verify ALL FOUR
   viewports + re-read every `console-*.txt`. A CSS variable / type-token change affects
   every viewport.
8. Commit one-fix-per-commit with the breakpoint in the body
   (`fix(hero): clamp --text-display ... (caught at 390px during end-of-build audit)`).
   Push after every commit (standing rule).

**Shutdown**
9. `TaskStop(task_id: "<saved from step 3>")` — the script closes its own browser, but
   the Next.js dev-server task keeps running and wastes context. `TaskStop` is the only
