...
/src
  /app
    ...
    layout.tsx                   ← Root layout (fonts, metadataBase + OG metadata, StructuredData, analytics)
    /about/page.tsx
  ...
  /components
    StructuredData.tsx           ← Site-wide Person + Organization JSON-LD (sameAs) — rendered once in layout.tsx (Pattern #100)
...
**Default hero = Architecture B (movie header) where the business has a photographable subject** (product, space, job-site) — a full-bleed Higgsfield cinematic video loop. See the CLAUDE.md Hero Architecture Rule + [knowledge/patterns/higgsfield-movie-hero-pipeline.md](knowledge/patterns/higgsfield-movie-hero-pipeline.md) (Pattern #80) for the pipeline and Hero.tsx wire-in. **The two-column canvas layout below is Architecture A** — the default for abstract / service brands with no shootable subject, and the always-ready fallback when a movie hero can't pass the motion-readability gate.
...
## Site-Wide Entity Schema (Person + Organization `sameAs`) — DEFAULT scaffold

Every build ships site-wide structured data that establishes the brand as a single, citable entity. For personal-brand / solo-operator clients (coach, consultant, agent, attorney, therapist, speaker, creator — anyone where the *person* IS the business) this is **load-bearing**: it is the #1 thing that makes the owner findable for their own name in Google's Knowledge Graph and in AI answer engines (ChatGPT, Perplexity, Claude, Gemini, AI Overviews). It is an entity/identity layer, separate from per-page ranking schema. Scaffold it in Phase 1 — never retrofit at launch (the Andrea Abella Marie build shipped with none of it and the owner was invisible for her own name).

`src/components/StructuredData.tsx` — one server component, rendered ONCE in `app/layout.tsx <body>`:

```tsx
const sameAs = [
  "https://www.linkedin.com/in/<handle>",   // byte-exact live profile URLs from
  "https://www.instagram.com/<handle>",      // initial-business-data.md Section 5 —
  "https://facebook.com/<handle>",           // reuse the EXACT URLs wired into the footer
];
const graph = {
  "@context": "https://schema.org",
  "@graph": [
    { "@type": "Person", "@id": `${SITE}/#person`,
      name, jobTitle: POSITIONING, description: FULL_POSITIONING,
      image, telephone, email, knowsAbout: [...], worksFor: { "@id": ORG_ID }, sameAs },
    { "@type": ["Organization","ProfessionalService"], "@id": ORG_ID,
      name, founder: { "@id": PERSON_ID }, url, logo, telephone, email,
      areaServed: [...], sameAs },
  ],
};
...
Rules:
- `jobTitle` / `POSITIONING` = the ONE canonical positioning string captured at intake — byte-identical to the hero, footer subtitle, About lead, and the client's live social headlines.
- `sameAs` URLs must be **byte-exact** to the live profiles (a stale handle silently breaks the entity link). Reuse the footer URLs.
- Cross-reference `Person.worksFor` ↔ `Organization.founder` by `@id` so the two nodes resolve as one graph, not two orphans.
- Pair with the supporting scaffold (their absence also reads as "disjointed"): `metadataBase`, OpenGraph + Twitter card, canonical, `sitemap.ts`, `robots.ts`, self-contained `opengraph-image.tsx`.
- **No double-branding:** do not add a Next.js `title.template` if pages already embed the brand in their own titles — it double-brands. Brand appears once per surface (Pattern #93).
- A non-personal company (multi-owner, no single face) ships the `Organization` node + `sameAs` WITHOUT the `Person` node.

Populated by the seo-aeo-specialist agent in Stage 1F from the canonical string + `sameAs` URLs. Full pattern: `knowledge/patterns/cross-platform-entity-consistency-sameas.md` (Pattern #100).
...
- [ ] `/llms.txt` served (`src/app/llms.txt/route.ts`, generated from site.ts) — AI/agent-readability hygiene file. Honest framing: it's a Business-to-Agent convenience map for IDE/agent tools, NOT a ranking or AI-citation signal (major answer engines don't fetch it; Google compares it to the `keywords` meta tag). Cheap default, served `noindex`. Never sold to the client as a search-visibility win. (seo-aeo-specialist Step 6.5)
- [ ] Site-wide Person + Organization JSON-LD with `sameAs` present in the RENDERED HTML (personal-brand clients); canonical positioning string identical across site + live socials; each `sameAs` URL 200s to the real profile (Pattern #100)
...
- [ ] All screenshots saved to the single `/audits/` folder at project root (gitignored). Never scatter `audit-*.png` / `verify-*.png` across the root. Confirm `.gitignore` has `/audits/`. See the pattern doc's "Artifact organization" section.

**Capture — one command (vault disk script; Playwright MCP retired from the audit path)**
- [ ] `node "C:\Projects\Second Brain\Wealth\Optimus\tools\audit-capture.mjs" --out "<app>/audits" --url http://localhost:3000 --wait "<hero H1 tagline>"`
- [ ] `--wait` is the hero H1 (`siteConfig.tagline`) or a unique substring — post-hydration only (static shell text → blank screenshots)
- [ ] `manifest.json` shows all four viewports (desktop-1440 / mobile-390 / 375 / 428) succeeded

**Mobile nav drawer aria-label build standard (the capture script's default selectors depend on this — set it at scaffold time)**
- [ ] Mobile nav toggle carries `aria-label="Open navigation menu"`; its in-dialog close button carries `aria-label="Close navigation menu"`. (The script tolerates the shorter "Open/Close menu" too; pass `--nav-open-selector`/`--nav-close-selector` only for a build that deviates further.)

**Analysis — disk facts first, vision only when flagged (per the pattern doc)**
- [ ] `console-*.txt`: 0 errors, 0 warnings at every viewport
- [ ] `tones-*.txt`: strict D/L alternation, no DD/LL, ends opposite the footer (Pattern #98 / Error #79)
- [ ] `overflow-*.txt`: `overflow=false` at every viewport (esp. 375)
- [ ] `-fold.png` (viewport @2×): hero above the fold (eyebrow + H1 + tagline + primary CTA), Half-Eye squint lands H1 → CTA → proof, `mobile-390-nav-open.png` overlay dark/opaque + branding + CTA present
- [ ] `-full.png` (fullPage @1×): no H1 orphans, section rhythm intact, blog/gallery imagery on-subject + modern (Error #81)
- [ ] `-rm.png`: every section still a gradient under reduced-motion (never flat)
...
- [ ] Re-run `audit-capture.mjs` (regenerates all artifacts), then re-verify ALL
      affected breakpoints — not just the one where you caught it (a CSS variable
      change affects every viewport)
- [ ] Re-read every `console-*.txt` after the re-run
...
- [ ] `TaskStop(task_id)` — the capture script closes its own browser, but the Next.js
      dev-server background task keeps running and wastes context; `TaskStop` is the
      only way to actually kill it
